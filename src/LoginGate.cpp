...
bool LoginGate::authenticate(QString &outMasterPassword) {
  ...
  QString label =
      isFirstLaunch ? "Make New Master Password:" : "Enter Master Password:";
  ...
  if (isFirstLaunch) {
    ...
    if (!file.open(QIODevice::WriteOnly)) {
      QMessageBox::critical(nullptr, "Error",
                            "Could not write to master password file");
      return false;
    }
  ...
  if (!file.open(QIODevice::ReadOnly)) {
    QMessageBox::critical(nullptr, "Error",
                          "Could not read from master password file");
    return false;
  }
  ...
  for (int attempt = 1; attempt <= maxAttempts; ++attempt) {
    QString label = QString("Master Password (%1/%2 attempts): ")
                        .arg(attempt)
                        .arg(maxAttempts);
